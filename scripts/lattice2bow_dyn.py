  ...
  for state in states:
    ...
    for dst, tokenWts in arcs.items():
      for token, wt in tokenWts:
        ...
        expectedCounts[token].append(alpha[currState] + beta[dst] + wt - norm)
...
def test():
  test_lattice = load_lattice('/export/a01/abenton/mono0a_test_lattices/19960605_CNN_HDL-00589876-00604409.lat')
...
def main():
    ...
    parser.add_argument('--cost', action='store_true', help='weights are costs (positive values, may be >1)')
